...
// ─────────────────────────────────────────────────────────────
// Local color constants (replaces the broken OrgColors import)
// ─────────────────────────────────────────────────────────────
class _Colors {
  static const primaryLight = Color(0xFFFF6B00);
...
class _ApprovedEventsCalendarState extends State<ApprovedEventsCalendar> {
  ...
  Widget build(BuildContext context) {
    ...
    while (gridDays.length % 7 != 0) {
      gridDays.add(gridDays.last.add(const Duration(days: 1)));
    }
                        ...
                        Expanded(
                          child: SingleChildScrollView(
                            child: Column(
                              ...
                              children: dayEvents.take(4).map((doc) {
                                final data = doc.data()
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const SizedBox(height: 6),
            Text(
              'Date: ${data['date'] is Timestamp ? (data['date'] as Timestamp).toDate().toLocal().toString().split(' ').first : data['date']?.toString() ?? '—'}',
            ),
            const SizedBox(height: 6),
            Text('Time: ${data['time'] ?? '—'}'),
            const SizedBox(height: 6),
