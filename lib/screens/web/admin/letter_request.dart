// lib/screens/admin/letter_request.dart
...
class _AdminLetterRequestScreenState extends State<AdminLetterRequestScreen> {
  ...
  Widget _buildTableHeader() {
    return Container(
      ...
      child: Row(children: [
        Expanded(flex: 3, child: _headerCell('REQUESTOR (ORG)')),
        ...
        child: Row(children: [
          Expanded(
            ...
            child: Column(
              ...
              children: [
                Text(
                  data['orgName'] ?? 'Unknown Organization',
...
class _ExportButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return OutlinedButton.icon(
      ...
      style: OutlinedButton.styleFrom(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))),
    );
  }
  ...
  Future<void> _exportCSV(BuildContext context) async {
    try {
      ...
      if (searchTerm.isNotEmpty) docs = docs.where((d) {
        final data = d.data();
        return (data['orgName'] ?? '').toString().toLowerCase().contains(searchTerm) ||
