    ...
    child: Row(children: [
      ...
      Text(
        ...
        style: GoogleFonts.beVietnamPro(
          ...
          color: UpriseColors.primaryDark, letterSpacing: 0.3,
        ),
      ),
...
Widget _statusBadge(String status) {
  final Map<String, _BadgeStyle> styles = {
    'submitted': _BadgeStyle(UpriseColors.successBg, UpriseColors.success, 'SUBMITTED'),
  ...
  return Container(
    ...
    decoration: BoxDecoration(color: s.bg, borderRadius: BorderRadius.circular(_DS.radiusPill)),
    child: Text(s.label,
        style: GoogleFonts.beVietnamPro(
            fontSize: 10, fontWeight: FontWeight.w700, color: s.fg, letterSpacing: 0.8)),
  );
}
...
class EventReport {
  ...
  final List<Map<String, dynamic>> incomeBreakdown, expenseBreakdown, attachments;
  final List<String> financialNotes, recommendations;

  EventReport({
    required this.id, required this.title, required this.orgId,
    required this.orgName, required this.type, required this.date, required this.status,
    this.totalIncome = 0, this.totalExpenses = 0, this.budgetVariance = 0,
    this.registrants = 0, this.attendees = 0,
    this.submittedBy = '', this.reportPeriod = '', this.submittedDate,
    this.incomeBreakdown = const [], this.expenseBreakdown = const [],
    this.attachments = const [], this.financialNotes = const [], this.recommendations = const [],
  });
  ...
  factory EventReport.fromFirestore(
      DocumentSnapshot doc, String orgName, int registrants, int attendees) {
    final d = doc.data() as Map<String, dynamic>;
    return EventReport(
      id: doc.id, title: d['title']?.toString() ?? 'Untitled',
      orgId: d['orgId']?.toString() ?? '', orgName: orgName,
      ...
      registrants: registrants, attendees: attendees,
...
class _ReportsManagementState extends State<ReportsManagement>
  ...
  void initState() {
    ...
    _tabController.addListener(() => setState(() { _detailEvent = null; _currentPage = 1; }));
    _loadOrganizations();
    _loadDeadlines();
    _loadSubmissionData();
    _loadEvents();
  ...
  Future<void> _loadOrganizations() async {
    ...
    setState(() {
      _organizations = snap.docs.map((doc) => {
        'id': doc.id, 'name': doc.data()['name']?.toString() ?? 'Unknown',
      }).toList();
    });
  }
  ...
  Future<void> _loadEvents() async {
    ...
    try {
      ...
      if (_filterType != 'All Types') query = query.where('type', isEqualTo: _filterType);
      if (_filterOrg != 'All Organizations') {
        final org = _organizations.firstWhere(
          (o) => o['name'] == _filterOrg, orElse: () => {'id': '', 'name': ''});
        if ((org['id'] as String).isNotEmpty) query = query.where('orgId', isEqualTo: org['id']);
      }
      ...
      for (final doc in eventsSnap.docs) {
        ...
        final regSnap = await FirebaseFirestore.instance
            .collection('registrations').where('eventId', isEqualTo: doc.id).get();
        final registrants = regSnap.docs.length;
        final attendees   = regSnap.docs.where((r) => (r.data())['attended'] == true).length;
  ...
  Future<void> _loadDeadlines() async {
    ...
    try {
      final doc = await FirebaseFirestore.instance.collection('report_deadlines').doc('deadlines').get();
      if (doc.exists) {
        final data = doc.data()!;
        _financialDeadline      = (data['financial'] as Timestamp?)?.toDate();
        _accomplishmentDeadline = (data['accomplishment'] as Timestamp?)?.toDate();
      }
    } catch (e) { debugPrint('Error loading deadlines: $e'); }
    finally { if (mounted) setState(() => _loadingDeadlines = false); }
  }

  Future<void> _saveDeadlines() async {
    if (_financialDeadline == null || _accomplishmentDeadline == null) return;
    await FirebaseFirestore.instance.collection('report_deadlines').doc('deadlines').set({
      'financial':      Timestamp.fromDate(_financialDeadline!),
      'accomplishment': Timestamp.fromDate(_accomplishmentDeadline!),
    });
    await activity_log.ActivityLogger.log(
      action: 'Updated report deadlines: Financial → ${DateFormat('yyyy-MM-dd').format(_financialDeadline!)}, '
  ...
  Future<void> _loadSubmissionData() async {
    await Future.wait([_loadFinancialSubmissions(), _loadAccomplishmentSubmissions()]);
  }
    ...
    try {
      ...
      for (final doc in subsSnap.docs) {
        ...
        subsMap[data['orgId']?.toString() ?? ''] = {
          ...
          'fileUrl': data['fileUrl'], 'submissionId': doc.id,
        };
      }
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _logGeneratedReport(String fileName, String format, String type) async {
    await FirebaseFirestore.instance.collection('generated_reports').add({
      'fileName': fileName, 'dateRange': _filterRange, 'organization': _filterOrg,
      'eventType': _filterType, 'reportView': _reportView,
      'generatedAt': FieldValue.serverTimestamp(), 'format': format, 'reportType': type,
    });
    await activity_log.ActivityLogger.log(
      action: 'Generated $type report in $format format', module: 'Reports', severity: 'info',
  ...
  Future<void> _exportFinancialCSV() async {
    ...
    for (final e in _events) {
      rows.add([e.title, e.orgName, e.type, DateFormat('yyyy-MM-dd').format(e.date),
        e.totalIncome.toStringAsFixed(2), e.totalExpenses.toStringAsFixed(2), e.netAmount.toStringAsFixed(2)]);
    }
  ...
  Future<void> _exportSubmissionCSV(
      String reportType, List<OrgSubmission> submissions, DateTime? deadline) async {
    final rows = <List<String>>[['Organization', 'Deadline', 'Submitted On', 'Status']];
    for (final sub in submissions) {
      final isSubmitted = sub.submittedAt != null;
      final isOverdue   = deadline != null && !isSubmitted && DateTime.now().isAfter(deadline);
      rows.add([sub.orgName,
  ...
  Widget _buildFinancialTab() {
    return Column(children: [
      Expanded(child: _buildEventsTable(showFinancial: true, showCountdown: false)),
      _buildRecentReportsBar(),
    ]);
  }

  // ── Events Table — full StudentAccounts-style implementation ──────
  Widget _buildEventsTable({required bool showFinancial, required bool showCountdown}) {
    if (_loadingEvents) return const Center(child: CircularProgressIndicator());
    final totalPages = _events.isEmpty ? 1 : (_events.length / _pageSize).ceil();
    final safePage   = _currentPage.clamp(1, totalPages);
    final start      = (safePage - 1) * _pageSize;
    final end        = (start + _pageSize).clamp(0, _events.length);
    final pageItems  = _events.isEmpty ? <EventReport>[] : _events.sublist(start, end);
    ...
    return Container(
      ...
      child: Column(children: [
        _buildTableHeader(showFinancial: showFinancial, showCountdown: showCountdown),
        Expanded(
  ...
  Widget _buildTableHeader({required bool showFinancial, required bool showCountdown}) {
    return Container(
      ...
      child: Row(children: showFinancial ? [
        ...
        if (showCountdown) Expanded(flex: 2, child: _headerCell('DAYS TO EVENT')),
  ...
  Widget _buildEventRow({
    required EventReport event, required bool isLast,
    required bool showFinancial, required bool showCountdown,
  }) {
    final now       = DateTime.now();
    final daysUntil = event.date.difference(now).inDays;
    final String countdownText  = daysUntil < 0 ? 'Passed' : daysUntil == 0 ? 'Today' : '$daysUntil days';
    final Color  countdownColor = daysUntil < 0
        ? const Color(0xFF9AA5B4)
        : daysUntil == 0 ? UpriseColors.success
        : daysUntil <= 7 ? UpriseColors.warning : UpriseColors.primaryDark;
      ...
      child: Container(
        ...
        child: Row(children: showFinancial ? [
          Expanded(flex: 3, child: Row(children: [
            _EventIcon(type: event.type), const SizedBox(width: 10),
            Expanded(child: Text(event.title,
                style: GoogleFonts.beVietnamPro(fontSize: 13, fontWeight: FontWeight.w600, color: const Color(0xFF1A202C)),
                overflow: TextOverflow.ellipsis)),
          ])),
          Expanded(flex: 2, child: Text(event.orgName,
              style: GoogleFonts.beVietnamPro(fontSize: 12, color: const Color(0xFF64748B)), overflow: TextOverflow.ellipsis)),
          Expanded(flex: 2, child: Text('₱${_fmt(event.totalIncome)}',
              style: GoogleFonts.beVietnamPro(fontSize: 13, fontWeight: FontWeight.w600, color: UpriseColors.success))),
          Expanded(flex: 2, child: Text('₱${_fmt(event.totalExpenses)}',
              style: GoogleFonts.beVietnamPro(fontSize: 13, fontWeight: FontWeight.w600, color: UpriseColors.error))),
          Expanded(flex: 2, child: Text('₱${_fmt(event.netAmount)}',
          ...
          Expanded(flex: 1, child: Align(alignment: Alignment.centerRight,
              child: _ActionIconButton(icon: Icons.visibility_outlined, tooltip: 'View Report',
                  onTap: () => setState(() => _detailEvent = event)))),
        ] : [
          Expanded(flex: 3, child: Row(children: [
            _EventIcon(type: event.type), const SizedBox(width: 10),
          ...
          Expanded(flex: 2, child: Text(DateFormat('MMM dd, yyyy').format(event.date),
              style: GoogleFonts.beVietnamPro(fontSize: 12, color: const Color(0xFF64748B)))),
          Expanded(flex: 1, child: Text('${event.registrants}',
              style: GoogleFonts.beVietnamPro(fontSize: 13, fontWeight: FontWeight.w600, color: const Color(0xFF1A202C)))),
          ...
          Expanded(flex: 2, child: Row(children: [
            Expanded(child: ClipRRect(
              ...
              child: LinearProgressIndicator(
                ...
                color: UpriseColors.primaryDark, minHeight: 6),
            )),
            const SizedBox(width: 6),
            Text('${event.attendanceRatio}%',
  ...
  Widget _buildEmptyState() {
    return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      Container(width: 80, height: 80,
        ...
        child: const Icon(Icons.bar_chart_rounded, size: 40, color: Color(0xFF9AA5B4))),
      const SizedBox(height: 16),
      Text('No events found', style: GoogleFonts.beVietnamPro(fontSize: 15, fontWeight: FontWeight.w600, color: const Color(0xFF374151))),
  ...
  Widget _buildTableFooter(int total, int totalPages, int start, int end) {
    ...
    return Container(
      ...
      child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        Text('Showing ${total == 0 ? 0 : start + 1}–$end of $total events',
            style: GoogleFonts.beVietnamPro(fontSize: 12, color: const Color(0xFF64748B))),
        Row(children: [
    ...
    return Container(
      ...
      child: Column(children: [
        Container(
          ...
          child: Row(children: [
            const Icon(Icons.history_rounded, size: 16, color: UpriseColors.primaryDark),
            const SizedBox(width: 8),
        ...
        StreamBuilder<QuerySnapshot>(
          stream: FirebaseFirestore.instance.collection('generated_reports')
              .orderBy('generatedAt', descending: true).limit(5).snapshots(),
          builder: (ctx, snap) {
            if (!snap.hasData) {
            ...
            if (snap.data!.docs.isEmpty) {
              return Padding(padding: const EdgeInsets.all(20),
                  child: Text('No reports generated yet.',
                      style: GoogleFonts.beVietnamPro(color: const Color(0xFF64748B), fontSize: 13)));
            }
            return Column(children: snap.data!.docs.asMap().entries.map((entry) {
              final i   = entry.key;
              ...
              return Container(
                ...
                child: Row(children: [
                  Container(width: 32, height: 32,
                    decoration: BoxDecoration(
                      color: isPdf ? UpriseColors.errorBg : UpriseColors.successBg,
                      borderRadius: BorderRadius.circular(8)),
                    child: Icon(isPdf ? Icons.picture_as_pdf_rounded : Icons.table_chart_rounded,
                        size: 16, color: isPdf ? UpriseColors.error : UpriseColors.success)),
                  const SizedBox(width: 12),
                  Expanded(child: Text(d['fileName'] ?? 'Report',
                      style: GoogleFonts.beVietnamPro(fontSize: 13, fontWeight: FontWeight.w500, color: const Color(0xFF1A202C)))),
                  Text(generatedAt != null ? DateFormat('MMM dd, yyyy hh:mm a').format(generatedAt) : '—',
                      style: GoogleFonts.beVietnamPro(fontSize: 12, color: const Color(0xFF64748B))),
                  const SizedBox(width: 12),
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                    ...
                    child: Text(d['format'] ?? '',
                        style: GoogleFonts.beVietnamPro(fontSize: 10, fontWeight: FontWeight.w700,
                            color: isPdf ? UpriseColors.error : UpriseColors.success, letterSpacing: 0.8)),
                  ),
                ]),
              );
            }).toList());
  ...
  Widget _buildSubmissionTrackerTab() {
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        _sectionLabel('Financial Report Submissions', icon: Icons.attach_money_rounded),
        _buildSubmissionTable('Financial', _financialSubs, _financialDeadline, _loadingFinancial),
        const SizedBox(height: 24),
        _sectionLabel('Accomplishment Report Submissions', icon: Icons.assignment_rounded),
  ...
  Widget _buildDeadlineBar() {
    ...
    return Container(
      ...
      child: Row(children: [
        const Icon(Icons.timer_outlined, color: UpriseColors.primaryDark, size: 18),
        const SizedBox(width: 10),
        Expanded(child: Wrap(spacing: 24, runSpacing: 8, children: [
          _deadlineInfo('Financial Deadline', _financialDeadline),
          _deadlineInfo('Accomplishment Deadline', _accomplishmentDeadline),
        ])),
        const SizedBox(width: 16),
        _ToolbarButton(label: 'Edit Deadlines', icon: Icons.edit_calendar_rounded,
            onPressed: _showDeadlineDialog, outlined: true),
      ]),
    );
  }
  ...
  Widget _deadlineInfo(String label, DateTime? date) {
    if (date == null) {
      return Row(mainAxisSize: MainAxisSize.min, children: [
        ...
        Text('Not set', style: GoogleFonts.beVietnamPro(fontSize: 13, color: const Color(0xFF9AA5B4))),
      ]);
    }
    ...
    return Row(mainAxisSize: MainAxisSize.min, children: [
      Text('$label: ', style: GoogleFonts.beVietnamPro(fontSize: 13, fontWeight: FontWeight.w600)),
      Text(DateFormat('MMM dd, yyyy').format(date), style: GoogleFonts.beVietnamPro(fontSize: 13)),
      const SizedBox(width: 8),
      Container(
        padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(_DS.radiusPill)),
        child: Text(overdue ? 'Overdue' : '⏱ $days days left',
            style: GoogleFonts.beVietnamPro(fontSize: 10, fontWeight: FontWeight.w700,
                color: overdue ? UpriseColors.error : UpriseColors.warning, letterSpacing: 0.5)),
      ),
    ]);
  }
  ...
  Widget _buildTrackerStats() {
    ...
    return Padding(
      ...
      child: Row(children: [
        _StatCard(label: 'Total Organizations',         value: '$total',                         icon: Icons.business_rounded,              color: UpriseColors.primaryDark),
        const SizedBox(width: 14),
        _StatCard(label: 'Financial Submitted',         value: '$finSubmitted/$total',            icon: Icons.attach_money_rounded,          color: UpriseColors.success),
        const SizedBox(width: 14),
        _StatCard(label: 'Accomplishment Submitted',    value: '$accSubmitted/$total',            icon: Icons.assignment_turned_in_rounded,  color: UpriseColors.info),
        const SizedBox(width: 14),
        _StatCard(label: 'Pending',
            value: '${(total - finSubmitted) + (total - accSubmitted)}',
            icon: Icons.pending_actions_rounded, color: UpriseColors.warning),
      ]),
    );
  }

  Widget _buildSubmissionTable(
      String title, List<OrgSubmission> submissions, DateTime? deadline, bool loading) {
    if (loading) return const Padding(padding: EdgeInsets.all(32), child: Center(child: CircularProgressIndicator()));

    final sorted = List<OrgSubmission>.from(submissions)..sort((a, b) {
      final now = DateTime.now();
              ...
              child: Row(children: [
                ...
                Expanded(flex: 2, child: isSubmitted ? _statusBadge('submitted')
                    ...
                    : Row(children: [
                        ...
                        Text(daysLeft > 0 ? '$daysLeft d left' : 'today',
                            style: GoogleFonts.beVietnamPro(fontSize: 10, color: UpriseColors.warning, fontWeight: FontWeight.w600)),
                      ])),
                Expanded(flex: 1, child: Align(alignment: Alignment.centerRight,
                    child: isSubmitted
        ...
        Container(
          ...
          child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            Text('Showing ${sorted.length} organizations',
                style: GoogleFonts.beVietnamPro(fontSize: 12, color: const Color(0xFF64748B))),
            _ToolbarButton(label: 'Export CSV', icon: Icons.download_rounded,
                onPressed: () => _exportSubmissionCSV(title, submissions, deadline), outlined: true),
          ]),
        ),
      ]),
    );
      ...
      body: Column(children: [
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            GestureDetector(
              ...
              child: Row(children: [
                ...
                Text('Back to Reports List',
                    style: GoogleFonts.beVietnamPro(fontSize: 13, color: const Color(0xFF64748B), fontWeight: FontWeight.w500)),
              ]),
            ),
            ...
            Row(children: [
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(event.title,
                    style: GoogleFonts.beVietnamPro(fontSize: 20, fontWeight: FontWeight.w800, color: const Color(0xFF1A202C))),
                Text('${event.orgName} • ${event.type} • ${DateFormat('MMM dd, yyyy').format(event.date)}',
              ...
              Row(children: [
                ...
                ElevatedButton.icon(
                  ...
                  label: Text('Download PDF', style: GoogleFonts.beVietnamPro(fontSize: 13, fontWeight: FontWeight.w600)),
                  style: ElevatedButton.styleFrom(
                    backgroundColor: UpriseColors.primaryDark, foregroundColor: Colors.white, elevation: 0,
                    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(_DS.radiusSm)),
                    padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10)),
                ),
              ]),
        ...
        Expanded(child: SingleChildScrollView(
          ...
          child: Column(children: [
            ...
            // Financial stats
            Row(children: [
              _detailStatCard('Total Income',   '₱${_fmt(event.totalIncome)}',  UpriseColors.success, UpriseColors.successBg, Icons.trending_up_rounded),
              const SizedBox(width: 14),
              _detailStatCard('Total Expenses', '₱${_fmt(event.totalExpenses)}',UpriseColors.error,   UpriseColors.errorBg,   Icons.trending_down_rounded),
              const SizedBox(width: 14),
              _detailStatCard('Net Amount',     '₱${_fmt(net)}',
                  net >= 0 ? UpriseColors.success : UpriseColors.error,
                  net >= 0 ? UpriseColors.successBg : UpriseColors.errorBg,
                  Icons.account_balance_wallet_rounded),
              const SizedBox(width: 14),
              _detailStatCard('Budget Variance','₱${_fmt(event.budgetVariance)}', UpriseColors.primaryDark, UpriseColors.primaryLight, Icons.balance_rounded),
            ]),
            const SizedBox(height: 16),
            // Breakdowns
            Row(children: [
              Expanded(child: _breakdownCard('Income Breakdown',  event.incomeBreakdown,  maxInc, UpriseColors.success, Icons.trending_up_rounded)),
              const SizedBox(width: 14),
              Expanded(child: _breakdownCard('Expense Breakdown', event.expenseBreakdown, maxExp, UpriseColors.error,   Icons.trending_down_rounded)),
            ]),
            const SizedBox(height: 14),
            Row(children: [
              Expanded(child: _notesCard('Financial Notes',  event.financialNotes,  Icons.notes_rounded)),
              const SizedBox(width: 14),
              Expanded(child: _notesCard('Recommendations',  event.recommendations, Icons.lightbulb_outline_rounded)),
            ]),
            if (event.attachments.isNotEmpty) ...[const SizedBox(height: 14), _attachmentsCard(event.attachments)],
            const SizedBox(height: 24),
          ]),
        )),
      ]),
  ...
  Widget _detailStatCard(String label, String value, Color color, Color bgColor, IconData icon) {
    return Expanded(child: Container(
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(value, style: GoogleFonts.beVietnamPro(fontSize: 18, fontWeight: FontWeight.w800, color: color)),
        ])),
      ]),
    ));
  }

  Widget _breakdownCard(String title, List<Map<String, dynamic>> items, double maxVal, Color color, IconData icon) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (items.isEmpty)
          Text('No breakdown data.', style: GoogleFonts.beVietnamPro(color: const Color(0xFF64748B), fontSize: 13))
        else
          ...items.map((item) {
            ...
            return Padding(padding: const EdgeInsets.only(bottom: 12),
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  Text(item['name']?.toString() ?? '', style: GoogleFonts.beVietnamPro(fontSize: 13, color: const Color(0xFF64748B))),
                  Text('₱${_fmt(amt)}', style: GoogleFonts.beVietnamPro(fontSize: 13, fontWeight: FontWeight.w600)),
                ]),
                const SizedBox(height: 5),
                ClipRRect(borderRadius: BorderRadius.circular(3),
                    child: LinearProgressIndicator(value: ratio.toDouble(),
                        backgroundColor: const Color(0xFFE8ECF0), color: color, minHeight: 5)),
              ]));
          }),
      ]),
    );
  ...
  Widget _notesCard(String title, List<String> notes, IconData icon) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _sectionLabel(title, icon: icon),
        if (notes.isEmpty)
          Text('No entries recorded.', style: GoogleFonts.beVietnamPro(color: const Color(0xFF64748B), fontSize: 13))
        else
  ...
  Widget _attachmentsCard(List<Map<String, dynamic>> attachments) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Wrap(spacing: 10, runSpacing: 10, children: attachments.map((att) {
            ...
            child: Container(
              ...
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                ...
                Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Text(size, style: GoogleFonts.beVietnamPro(fontSize: 11, color: const Color(0xFF64748B))),
                ]),
                const SizedBox(width: 10),
  ...
  Widget _metaCell(String key, String value, {bool last = false, bool lastRow = false}) {
    return Expanded(child: Container(
      ...
      decoration: BoxDecoration(border: Border(
        right:  last    ? BorderSide.none : const BorderSide(color: Color(0xFFE8ECF0)),
        bottom: lastRow ? BorderSide.none : const BorderSide(color: Color(0xFFE8ECF0)),
      )),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(key,   style: GoogleFonts.beVietnamPro(fontSize: 10, fontWeight: FontWeight.w700, color: const Color(0xFF64748B), letterSpacing: 0.5)),
        const SizedBox(height: 4),
        Text(value, style: GoogleFonts.beVietnamPro(fontSize: 13, fontWeight: FontWeight.w600, color: const Color(0xFF1A202C))),
      ]),
    ));
  }
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, setDlg) => Dialog(
        ...
        child: SizedBox(width: 460, child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Padding(padding: const EdgeInsets.all(24), child: Column(children: [
            _deadlineTile('Financial Report Deadline',      tempFin, Icons.attach_money_rounded,     ctx, (p) => setDlg(() => tempFin = p)),
            const SizedBox(height: 12),
            _deadlineTile('Accomplishment Report Deadline', tempAcc, Icons.assignment_rounded,        ctx, (p) => setDlg(() => tempAcc = p)),
          ])),
          // Footer
          Container(
            ...
            child: Row(mainAxisAlignment: MainAxisAlignment.end, children: [
              OutlinedButton(onPressed: () => Navigator.pop(ctx),
                style: OutlinedButton.styleFrom(side: const BorderSide(color: Color(0xFFE2E6EA)),
              ...
              ElevatedButton(
                onPressed: () async {
                  setState(() { _financialDeadline = tempFin; _accomplishmentDeadline = tempAcc; });
                ...
                child: Text('Save Deadlines', style: GoogleFonts.beVietnamPro(fontSize: 13, fontWeight: FontWeight.w600))),
            ]),
          ),
        ])),
      )),
  ...
  Widget _deadlineTile(String label, DateTime? date, IconData icon, BuildContext ctx, void Function(DateTime) onPicked) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14),
      decoration: BoxDecoration(color: const Color(0xFFF8F9FB), borderRadius: BorderRadius.circular(_DS.radiusSm),
          border: Border.all(color: const Color(0xFFE2E6EA))),
      child: Row(children: [
        Container(width: 36, height: 36,
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(date != null ? DateFormat('MMMM dd, yyyy').format(date) : 'Not set',
              style: GoogleFonts.beVietnamPro(fontSize: 13,
                  color: date != null ? const Color(0xFF1A202C) : const Color(0xFF9AA5B4))),
        ])),
        IconButton(
          icon: const Icon(Icons.calendar_today_rounded, color: UpriseColors.primaryDark, size: 18),
          onPressed: () async {
            final picked = await showDatePicker(context: ctx,
                initialDate: date ?? DateTime.now(), firstDate: DateTime.now(),
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, setDlg) => Dialog(
        ...
        child: SizedBox(width: 440, child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Padding(padding: const EdgeInsets.all(24), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            _sectionLabel('Report Parameters', icon: Icons.tune_rounded),
            _summaryRow('Date Range',   _filterRange), const SizedBox(height: 8),
            _summaryRow('Organization', _filterOrg),  const SizedBox(height: 8),
            _summaryRow('Event Type',   _filterType),  const SizedBox(height: 8),
            _summaryRow('Report View',  _reportView),
            const SizedBox(height: 20),
            _sectionLabel('Output Format', icon: Icons.file_download_rounded),
            Row(children: [
              Expanded(child: GestureDetector(onTap: () => setDlg(() => fmt = 'PDF'),
                  child: _formatOption('PDF', Icons.picture_as_pdf_rounded, UpriseColors.error,   UpriseColors.errorBg,   fmt == 'PDF'))),
              const SizedBox(width: 12),
              Expanded(child: GestureDetector(onTap: () => setDlg(() => fmt = 'CSV'),
                  child: _formatOption('CSV', Icons.table_chart_rounded,     UpriseColors.success, UpriseColors.successBg, fmt == 'CSV'))),
            ]),
          ])),
          Container(
            padding: const EdgeInsets.fromLTRB(24, 0, 24, 20),
  ...
  Widget _formatOption(String label, IconData icon, Color color, Color bgColor, bool selected) {
    return Container(
      padding: const EdgeInsets.all(14),
      decoration: BoxDecoration(
        color: selected ? bgColor : const Color(0xFFF8F9FB),
        borderRadius: BorderRadius.circular(_DS.radiusSm),
        border: Border.all(color: selected ? color : const Color(0xFFE2E6EA), width: selected ? 1.5 : 1)),
      child: Row(children: [
        Icon(icon, color: selected ? color : const Color(0xFF9AA5B4), size: 20),
        const SizedBox(width: 10),
        Text(label, style: GoogleFonts.beVietnamPro(fontSize: 13, fontWeight: FontWeight.w600,
  ...
  Widget _summaryRow(String label, String value) => Row(children: [
    SizedBox(width: 120, child: Text(label, style: GoogleFonts.beVietnamPro(fontSize: 12, color: const Color(0xFF64748B)))),
      ...
      builder: (ctx) => Dialog(
        ...
        child: Container(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              Container(width: 42, height: 42,
                decoration: BoxDecoration(color: UpriseColors.successBg, borderRadius: BorderRadius.circular(10)),
                child: const Icon(Icons.assignment_turned_in_rounded, color: UpriseColors.success, size: 20)),
              const SizedBox(width: 14),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('$reportType Report', style: GoogleFonts.beVietnamPro(fontSize: 17, fontWeight: FontWeight.w700, color: const Color(0xFF1A202C))),
                Text(sub.orgName, style: GoogleFonts.beVietnamPro(fontSize: 13, color: const Color(0xFF64748B))),
              ])),
            ]),
            const SizedBox(height: 20),
            _credentialRow(label: 'Submitted On',
                value: DateFormat('MMMM dd, yyyy HH:mm').format(sub.submittedAt!),
                icon: Icons.calendar_today_rounded),
            const SizedBox(height: 12),
            _credentialRow(label: 'File',
                value: sub.fileUrl != null && sub.fileUrl!.isNotEmpty ? sub.fileUrl! : 'No file attached.',
  ...
  Widget _credentialRow({required String label, required String value, required IconData icon}) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        Icon(icon, size: 14, color: const Color(0xFF64748B)), const SizedBox(width: 6),
        Text(label, style: GoogleFonts.beVietnamPro(fontSize: 11, fontWeight: FontWeight.w600,
  ...
  String _fmtK(double value) {
    if (value.abs() >= 1000000) return '${NumberFormat('#,##0.0', 'en_PH').format(value / 1000000)}M';
    if (value.abs() >= 1000)    return '${NumberFormat('#,##0.0', 'en_PH').format(value / 1000)}K';
    ...
    return Expanded(child: Container(
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(label, style: GoogleFonts.beVietnamPro(fontSize: 11, color: const Color(0xFF64748B), fontWeight: FontWeight.w500)),
          const SizedBox(height: 2),
          Text(value, style: GoogleFonts.beVietnamPro(fontSize: 22, fontWeight: FontWeight.w700, color: const Color(0xFF1A202C))),
        ])),
      ]),
    ));
  }
  ...
  Widget build(BuildContext context) {
    if (outlined) {
      return OutlinedButton.icon(
        ...
        style: OutlinedButton.styleFrom(
          foregroundColor: UpriseColors.primaryDark, side: BorderSide(color: UpriseColors.primaryDark),
          padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10),
    ...
    return ElevatedButton.icon(
      ...
      label: Text(label, style: GoogleFonts.beVietnamPro(fontSize: 13, fontWeight: FontWeight.w600)),
      style: ElevatedButton.styleFrom(
        backgroundColor: UpriseColors.primaryDark, foregroundColor: Colors.white,
        padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10),
...
class _EventIcon extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final Map<String, _IconStyle> styles = {
      'Seminar':     _IconStyle(const Color(0xFFEFF6FF), const Color(0xFF2563EB), Icons.mic_rounded),
      'Workshop':    _IconStyle(const Color(0xFFFFF7ED), const Color(0xFFEA580C), Icons.build_rounded),
      'Exhibition':  _IconStyle(const Color(0xFFF5F3FF), const Color(0xFF7C3AED), Icons.museum_rounded),
      'Social':      _IconStyle(const Color(0xFFFCE7F3), const Color(0xFFDB2777), Icons.people_rounded),
      'Cultural':    _IconStyle(const Color(0xFFF0FDF4), const Color(0xFF16A34A), Icons.celebration_rounded),
      'Competition': _IconStyle(const Color(0xFFFEFCE8), const Color(0xFFCA8A04), Icons.emoji_events_rounded),
    };
    final s = styles[type] ?? _IconStyle(const Color(0xFFF3F4F6), const Color(0xFF6B7280), Icons.event_rounded);
...
class _TypeBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
      decoration: BoxDecoration(color: UpriseColors.infoBg, borderRadius: BorderRadius.circular(_DS.radiusPill)),
