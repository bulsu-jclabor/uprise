...
class _OrgReportsScreenState extends State<OrgReportsScreen> {
    ...
    return Scaffold(
      ...
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snap) {
          ...
          final all      = (snap.data?.docs ?? []).map(ReportModel.fromFirestore).toList();
  ...
  Widget _buildStatsRow(List<ReportModel> all) {
    ...
    return Padding(
      ...
      child: Row(children: [
        _StatCard(label: 'Total Reports',          value: '$total',     icon: Icons.article_outlined,             color: _DS.primary),
        const SizedBox(width: 14),
        _StatCard(label: 'Financial',              value: '$financial', icon: Icons.account_balance_outlined,     color: const Color(0xFF059669)),
        const SizedBox(width: 14),
  ...
  Widget _buildDeadlineRow(List<ReportModel> all) {
    DateTime? latestFinancial = all
        ...
        .fold<DateTime?>(null, (l, d) => l == null ? d : (d.isAfter(l) ? d : l));
  ...
  // ── Table ──────────────────────────────────────────────────────────────────
  Widget _buildTable(
      List<ReportModel> filtered, AsyncSnapshot<QuerySnapshot> snap) {
    ...
    final pageItems =
        filtered.isEmpty ? <ReportModel>[] : filtered.sublist(start, end);
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text('Try adjusting your filters or upload a new report.',
              style: GoogleFonts.beVietnamPro(
                  fontSize: 13, color: const Color(0xFF64748B))),
        ],
      ),
  ...
  void _openEditModal(ReportModel r) => showDialog(
        ...
        barrierColor: Colors.black54,
        builder: (_) =>
            _ReportModal(orgId: widget.orgId, existingReport: r),
      );
  ...
  Future<void> _refreshSubmissionStateAfterDelete(String type) async {
    ...
    await doc.set({
      ...
      'submittedAt': latest.data()['submittedAt'] ??
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(children: [
                    Expanded(
                      child: _detailItem('Report ID', report.reportId,
                  ...
                  if (report.description.isNotEmpty) ...[
                    const SizedBox(height: 14),
                    _detailItem('Description', report.description,
                        Icons.notes_rounded),
                  ],
                  if (report.fileUrl?.isNotEmpty == true) ...[
                    const SizedBox(height: 14),
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (isEdit) {
        ...
        if (oldType != _type) {
          // refresh old type's submission record
          final snap = await col
              .where('orgId', isEqualTo: widget.orgId)
              .where('type', isEqualTo: oldType)
              .get();
          final oldCol = FirebaseFirestore.instance
              .collection(_submissionCollection(oldType));
          if (snap.docs.isEmpty) {
            try { await oldCol.doc(widget.orgId).delete(); } catch (_) {}
          }
        }
        await activity_log.ActivityLogger.log(
          action: 'edit_report',
        ...
        final snap = await col
            ...
            .get();
        final nextNum =
            (snap.docs.length + 1).toString().padLeft(3, '0');
                    ...
                    children: [
                      ...
                      TextFormField(
                        ...
                        decoration: _DS.inputDecoration(
                          'Report Title',
                          hint: 'e.g. Q1 2026 Financial Summary',
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Column(children: [
          ...
          const SizedBox(height: 4),
          Text('PDF, DOC, DOCX, XLSX, JPG, PNG — max 10 MB',
              style: GoogleFonts.beVietnamPro(
                  fontSize: 11,
...
class _DeadlineCard extends StatelessWidget {
  ...
  const _DeadlineCard(
      {required this.label, this.deadline, this.submittedOn});
              ...
              children: [
                ...
                Text('Deadline: $deadlineText',
                    style: GoogleFonts.beVietnamPro(
                        fontSize: 12,
                        color: const Color(0xFF64748B))),
                Text('Last upload: $submittedText',
                    style: GoogleFonts.beVietnamPro(
                        fontSize: 12,
                        color: const Color(0xFF64748B))),
              ],
    ...
    try {
      ...
      if (format == 'csv') {
        final csv = [headers, ...dataRows]
            ...
            .join('\n');
        await OrgExportUtil.saveText(csv, 'reports_$now.csv',
            mimeType: 'text/csv');
      } else {
        ...
        await OrgExportUtil.saveBytes(pdfBytes, 'reports_$now.pdf',
